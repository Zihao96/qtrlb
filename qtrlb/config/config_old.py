...
class Config:
    ...
    def set(self, key, value, reload_config=False, save_config=False):
        ...
        Config.replace_vars(self._config_dict, self._variables)
        ...
        if isinstance(dic, dict):
            for key in dic:
                if isinstance(dic[key], dict):
                    Config.replace_vars(dic[key], variables, max_depth=max_depth, depth=depth)
        ...
        # or a list, which we need to iterate over
            ...
            for i, item in enumerate(dic):
                if not isinstance(item, collections.Hashable):
                    Config.replace_vars(item, variables, max_depth=max_depth, depth=depth)
                else:
                    try:
