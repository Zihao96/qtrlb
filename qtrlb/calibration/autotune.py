...
def autotune(
    ...
    """
    if show_plot < require_confirmation:
        raise ValueError('Autotune: show_plot must be True if require_confirmation')
    ...
    if require_confirmation:
        ...
        if not confirmation: return ramsp, ramsn, None, None
    ...
    if require_confirmation:
        plt.show()
        confirmation = input('Autotune: press 1 to update parameter, any others to cancel.').startswith('1')
        if not confirmation: return ramsp, ramsn, das, None
    ...
    if require_confirmation:
        plt.show()
        confirmation = input('Autotune: press 1 to update parameter, any others to cancel.').startswith('1')
        if not confirmation: return ramsp, ramsn, das, dws
